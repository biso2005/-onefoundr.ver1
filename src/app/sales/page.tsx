...
import { Zap, FileText, Search, Share2, Mail, MessageSquare, Send, BarChart2 } from 'lucide-react'
import NewsletterCTA from '@/components/NewsletterCTA'
import { pillarMeta, gatewayHub, tracks, moatHubs } from '@/data/sales'
...
const trackGradients: Record<string, string> = {
  awareness: 'from-amber-500 to-orange-600',
  conversion: 'from-orange-400 to-rose-500',
  retention: 'from-teal-600 to-emerald-400',
}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {( [gatewayHub, ...tracks.flatMap(t => t.hubs)] ).map((hub) => {
              const Icon = trackIcons[hub.track] ?? FileText
      ...
      <section className="py-16 md:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-12">
            ...
            <p className="text-gray-500 max-w-2xl">Work through them in order or jump directly to the hub you need.</p>
          </div>

          <div className="space-y-10">
            {tracks.map((track) => {
              const Icon = trackIcons[track.id] ?? FileText
